...
def run(payload: dict[str, Any], *, now: datetime | None = None) -> dict[str, Any]:
    """Create the weekly reports and notify Telegram of the terminal outcome."""
    ...
    try:
        ...
        failure_code = "notion_report_create_failed"
        ...
        failure_code = "notion_report_invalid_response"
...
def _try_send_failure_alert(exc: Exception, *, fallback_code: str) -> None:
    ...
    try:
        _send_notification(title=FAILURE_TITLE, description=description, alert=True)
    except Exception:
        # Preserve the report failure as the service result when Telegram is also unavailable.
        pass
