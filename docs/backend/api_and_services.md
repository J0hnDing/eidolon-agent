...
Canonical effects determine read-only/destructive projections, and canonical `HIGH` risk is the only integration per-invocation approval trigger. Approved execution re-enters the same policy and becomes stale if provider account, caller authority, resource scope, risk, effects, schemas, or contract changes. Existing provider behavior remains unchanged: Calendar and Gmail share an OS-stored OAuth client but keep independent grants/accounts; Notion fixes separate Todo and Report data sources; Telegram separates notification execution from approval delivery; Atlas uses its bounded native API and Knowledge orchestration. `SecretStore` has no unsafe fallback. See the provider-specific integration documents for transport and connection details.
