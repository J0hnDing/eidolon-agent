...
- `/settings/integrations/google-calendar`: sanitized OAuth connection status and local disconnect. `/settings/integrations/google-calendar/oauth/start` accepts write-only OAuth client credentials and returns Google's authorization URL; the hidden callback consumes a single-use ten-minute state, installs the validated refresh-token bundle, and redirects to the Integrations Settings page with a bounded result marker.
