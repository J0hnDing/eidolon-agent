...
- `POST /act/workspace/open-root`: open the fixed local Act root; accepts no path input.
...
- `/functions/catalog`: the unified backend-core, user, and integration function catalog with availability state, reasons, and UI-only active-running projection for the Functions page.
...
- `/settings/integrations/microsoft`: sanitized Microsoft OAuth client status plus write-only client-id/secret configure/remove routes. The secret is stored only in the OS secret store.
- `/settings/integrations/outlook`: sanitized Outlook connection status, OAuth start/callback, reconnect, and disconnect. OAuth uses the Microsoft `common` authority, PKCE, account selection, and the bounded mail scopes; callbacks return only a result marker.
...
- `/settings/integrations/wecom`: sanitized WeCom Observer Bot ID/status, multi-user binding state, add-user pairing, per-user removal, write-only Secret replacement, and disconnect. WeCom is a backend-owned WebSocket transport with no callable integration operations or authorization UI.
...
- `/schedules`: generated and backend-owned service schedule list/edit/run-now operations; backend-owned availability is updated by stable service ID, while generated-service enable/disable uses the ordinary skill update route. Assistant assessment actions reuse the Agents assessment endpoints.
...
`FunctionCatalogService` persists one catalog projection at `runtime/function_catalog.json`, seeded with checked-in backend-core definitions and refreshed from the typed integration registry plus installed user-function contracts. Catalog reads are projection-only by default: startup performs installed-skill reconciliation and an initial rebuild, while skill lifecycle, runtime approval, active-version, and integration mutations rebuild the projection. `GET /functions/catalog?refresh=true` provides an explicit rebuild for the Capabilities UI Refresh action; ordinary UI polling performs no filesystem reconciliation, provider probing, catalog rewrite, or database commit. ProductManager receives only currently available agent-selectable entries as id/category/title/description/risk; Act-only MCP tools remain outside Project generation. The blueprint selects function ids. A Task DAG assigns those approved ids to nodes through `function_ids`; the backend resolves full schemas and invocation guidance into that node's `function_context`. The single-Codex workflow receives full context for every blueprint-selected function. Disabled user skills, invalid or stale function contracts, missing runtime approval, disconnected integrations, and Act-only tools remain visible in the UI but are not offered to ProductManager.

`PlatformServiceDispatcher` owns endpoint-only trusted backend dispatch. The daily Notion Done cleanup and daily Quercus knowledge sync are not in the function catalog and cannot be selected by ProductManager, another skill, or MCP. `SchedulerService` alone invokes them through trusted backend provider paths. They are registered from platform definitions as APScheduler jobs rather than `SkillSchedule` rows. Durable runtime state stores per-service availability and schedule overrides, and `/schedules/platform/{serviceId}` exposes bounded edit, availability, and run-now operations.
...
The checked-in `app/integrations` package separates discovery, authorization, and execution. `IntegrationOperationRegistry` owns semantic operation contracts and provider registration. `IntegrationAuthorizationService` owns standing skill grants, typed resource scopes, and fingerprints. `IntegrationCapabilityPolicy` rechecks the current caller, active manifest, permissions, authorization, resource, connection/account, schema, effects, and risk for every call. It alone can issue the immutable, secret-free `AuthorizedIntegrationInvocation` accepted by `IntegrationRuntime`. The runtime dispatches through a deterministic provider-adapter map and returns normalized output plus explicit audit-resource metadata. Provider adapters load credentials only after authorization and never decide caller authority. `IntegrationService` remains the Settings/connection/OAuth compatibility façade; catalog execution no longer uses its former centralized provider branch.

Canonical effects determine read-only/destructive projections, and canonical `HIGH` risk is the only integration per-invocation approval trigger. Approved execution re-enters the same policy and becomes stale if provider, default connection, account, caller authority, resource scope, risk, effects, schemas, or contract changes. The five email operations are provider-neutral: search and unread operations require one or both provider selectors, use per-provider page sizes and versioned composite cursors, merge newest-first results, and preserve provider-scoped errors and mark outcomes. Conversation reads and sends select exactly one provider. Existing provider behavior remains unchanged: Calendar and Gmail share an OS-stored OAuth client but keep independent grants/accounts; Notion fixes separate Todo and Report data sources plus one standalone Daily Feed page; Outlook uses a separate Microsoft client and Graph adapter; Telegram and WeCom use backend-owned transport delivery without exposing their chat transport as callable operations; Atlas uses its bounded native API and Knowledge orchestration. `SecretStore` has no unsafe fallback. See the provider-specific integration documents for transport and connection details.

Quercus is a trusted backend synchronization integration rather than an agent-callable operation. `QuercusService` owns connection identity, selected/retained course state, and account-scoped deleted-course exclusions. Deleting a tracked or untracked course removes it from future course listings and also deletes its mirror and synchronization rows when present. `QuercusSyncService` mirrors readable course content into the Act knowledge tree with database-owned incremental metadata and atomic writes. `QuercusProcessingService` migrates originals into `files/raw/`, optionally converts them through a fixed local Marker/Surya/llama.cpp contract into `files/processed/`, and keeps conversion status independent from Canvas sync. The processing setting stores a user-selected absolute llama.cpp directory. A pass-scoped backend controller starts one keep-alive inference server through Marker’s Surya environment, supplies its validated loopback URL to every conversion, reuses it across the pass, and terminates only its exact process tree without Windows console-control events. Its lifecycle-owned dispatcher cooperatively interrupts Marker and inference on backend shutdown, keeps interrupted work pending for automatic resume, leaves unchanged failures terminal until the user explicitly queues them through `POST /settings/integrations/quercus/processing/reprocess-failed`, recovers stale state and temporary output at startup, and logs bounded process-level diagnostics without secrets. When Canvas forbids the bulk file index, synchronization discovers references from accessible course content and downloads permitted files individually without treating that incomplete discovery as deletion-authoritative. Its other settings routes are `GET|PUT|DELETE /settings/integrations/quercus`, `GET|PUT /settings/integrations/quercus/courses`, `DELETE /settings/integrations/quercus/courses/{courseId}`, and `GET|PUT /settings/integrations/quercus/processing`. See [Quercus synchronization](../integrations/quercus.md).
...
`ProposedSkillService` validates proposed skill packages and their already-provisioned dependency and function-graph contract, reads bounded text package files including web assets without descending into inaccessible transient directories, installs proposed skills, rejects or deletes proposed skills, and creates one initially disabled service schedule from its required manifest intent. Validation and install never invoke `pip`. Installation excludes build-only dependencies plus sandbox/Codex/test bookkeeping, recovers interrupted copies idempotently, and moves source cleanup outside active roots before treating it as best-effort. Deletion atomically stages both proposed and installed folders before database cleanup, so Windows ACL or lock failures cannot leave contradictory active lifecycle state; parent function contracts become `error`, and ancestor services are disabled. It does not expose a sample-skill or bare-record creation path.
...
Uses APScheduler to manage exactly one schedule per generated service and the fixed backend-owned platform registry containing Notion cleanup and Quercus synchronization. Functions and web applications cannot declare or receive schedules. Manifest-declared service intent is registered as a paused row during install.
Schedule-mutating API routes use the FastAPI lifespan's shared APScheduler instance with their request-scoped database session, so schedule edits and service enable/disable update the live scheduler immediately. Run Now requires the generated service to be enabled. Automatic execution remains enabled-only and every path rechecks runtime permissions, integrations, child-function availability, schema compatibility, and the per-skill operation guard.
