...
`act_sessions` stores each durable Codex thread id, title, origin, lifecycle status, and timestamps. New agent sessions begin as `New Chat`; after a successful turn, the native Codex App Server thread name is the canonical generated title and is projected to the session, with an existing Telegram topic updated on a best-effort basis. `act_turns` stores the user message, queued/running/terminal state, execution start/completion timestamps, live Codex turn id, concise activity, final answer or bounded failure, cancellation request, and optional transport-neutral delivery provider/target/status. Only queued turns are safe to retain across restart; running rows are recovered as interrupted.
...
`wecom_observer_bindings` stores the singleton WeCom connection's hashed short-lived pairing code and expiry. `wecom_observer_user_bindings` stores one row per paired private user and that user's canonical `current_session_id`; no session history is exposed through the transport. Pairing creates the active current session immediately. Clear conversation swaps the pointer before retiring the old session; archiving a current WeCom session through Chat creates and claims a replacement in the same database transaction. API session reads derive nullable `wecom_user_id` metadata from this pointer, falling back to the persisted originating WeCom delivery user for an old busy session after a pointer swap, rather than duplicating ownership on `act_sessions`. `wecom_inbound_messages` stores per-connection message IDs for duplicate suppression. WeCom Bot ID and connection metadata use the generic `integration_connections` row; the Bot Secret remains only in the operating-system secret store. User removal deletes only one binding. Disconnect removes pairing, user-binding, and deduplication rows without deleting Observer sessions.
