...
The Google Calendar panel shows sanitized connection state, verified account email, last validation, and the exact fixed OAuth redirect URI. OAuth client ID and secret inputs are password-style, write-only, cleared as soon as authorization starts, and never redisplayed. Connect or Replace leaves the page for Google's consent screen. The callback returns to `/settings/integrations` with only a bounded result marker; the page refreshes integration/catalog state, shows a sanitized success or failure message, and removes that marker from browser history. Disconnect deletes only Eidolon's local credential and connection. There is no calendar, event, synchronization, webhook, cache, secondary-calendar, or schedule UI.
