...
The frontend is a React/Vite TypeScript app under `frontend/src`. Its local development server uses the fixed origin `http://localhost:5174`, keeping it distinct from the separate Eidolon marketing site.
...
The application shell groups these destinations into Workspace, Capabilities, and Control navigation. It uses one persistent desktop sidebar and a horizontally scrollable compact navigation bar on narrower screens. Each destination has a text label and decorative icon, the current route remains visibly selected, and a keyboard skip link moves directly to page content. The four Capabilities pages expose a top-right Refresh action; Functions performs an explicit catalog rebuild while the other pages reload their current list.
...
Passive status presentation uses a small semantic dot with a plain-text label instead of a colored capsule. Color is never the only state signal. Skills, functions, and schedules with an active execution replace their normal state with a tooltip-labelled blue dot and the `running` label in the row's state column; its glow pulses without moving the row content. Page and message transitions use short motion with no workflow-level delay. `prefers-reduced-motion` disables non-essential animation, and focused controls retain an explicit visible ring.

Delete actions generally use a consistent square red trash-icon button. Each icon-only control retains a specific accessible name and matching hover tooltip that identifies the record being deleted. Skill Detail instead uses an explicit **Delete Skill** text button in its title bar. Like Chat's Send control, destructive buttons switch to a solid semantic background and contrasting text or icon color on hover.
...
Concurrent identical GETs share one in-flight request. This prevents React development-mode mount replays and overlapping page consumers from duplicating read-only backend work; completed responses are not cached, so later navigation and explicit refreshes still read current state.

## Chat Page

The shared conversation page supports four explicit, conversation-owned modes:
...
Conversation mode is fixed after creation. The sidebar keeps the conversation list above a bottom-anchored **New Chat** button. Clicking it opens a compact popover above the button with Project, Act, Observer, and Assistant creation options; Project uses amber emphasis and Act uses blue emphasis. Every conversation row plus the transcript header carries a neutral symbol-and-text mode marker. The transcript header uses its former descriptive-headline space for session creation time and whether the session belongs to a WeCom user, including that user's ID. Direct Chat mode is not supported. Legacy direct-chat rows in frontend storage are discarded rather than reinterpreted as Project requests. There is no in-conversation mode switch and no separate Act page; the legacy `/act` route redirects to `/chat`.
...
Observer and Assistant conversations bind to the same backend sessions shown in Agents, including sessions started through Telegram or assessments. Observer icons are green and Assistant icons are purple in both Chat and Agents. Selecting a mode from the **New Chat** popover creates a conversation with that fixed mode. The compact picker shows only each mode's icon and create label; Project, Act, Observer, and Assistant use amber, blue, green, and purple left-edge/text accents respectively. Escape or clicking outside the popover dismisses it.
...
Chat persistence is local frontend storage managed by `frontend/src/lib/chatStore.ts`. Every transcript is owned strictly by its conversation id: a temporarily unavailable selection renders a clean Project placeholder rather than falling back to another conversation, and the transcript subtree remounts when the selected id changes. Project conversations also store the pending generation request id while ProductManager is waiting for clarification so the user's next reply stays attached to the same request. Agent conversation titles remain `New Chat` until `ChatPage` receives the persisted `ActSession.title`; polling and session-list imports update both the transcript and sidebar from that backend value.
Users can delete Project, Act, Observer, or Assistant conversations from the conversation toolbar. Project deletion removes the local transcript and asks the backend to remove persisted message rows for the same frontend conversation id; Project-mode approval records remain available through the approval pages. Agent conversation deletion archives the shared backend session before removing the local transcript. Deleting a WeCom-owned Observer session creates, imports, and selects the fresh current session for the same WeCom user.
...
`ChatPage` retains route-level API orchestration, imports active backend Act, Observer, and Assistant sessions, reconciles archived or pruned bindings, and polls only the selected agent conversation while its turn is queued or running. Deleting an agent conversation archives its backend session first; active work can be cancelled from the shared composer. `features/chat/useChatConversations.ts` owns local conversation selection, creation, deletion, drafts, fixed modes, agent-session bindings, and message transitions; it persists the synthesized first conversation before accepting edits. `features/chat/ChatWorkspace.tsx` owns the sidebar, fixed mode display, transcript, approval cards, and composer presentation.
...
Lists skills in one list with their `function`, `service`, or `web_app` runtime. A row replaces its normal Status value with the `running` indicator while its bounded runtime executes or while its persistent web application instance is active. Each row opens Skill Detail, including web application rows; application launch remains on the Applications and Skill Detail pages. Blue row arrows use the same color as the active page indicator, remain hidden at rest, and reveal from left to right with a short slide-and-fade animation on hover or keyboard focus. Services show the same backend-owned enabled/disabled availability as other skills. User-facing creation goes through the proposed-skill workflow, not bare database record creation.

## Functions Page

`/functions` displays the backend-owned unified catalog. It includes backend-core, installed user, and integration functions in every state, with id, description, source, risk, version, availability, unavailability reasons, and the `running` indicator replacing the Availability value when a persisted function run or audit is active. Integration sources use the specific provider name, such as GitHub, Google Calendar, or Gmail, while the underlying catalog category remains `integration`. The page supports combined keyword, source, availability, and risk filters, polls the catalog so enablement, deletion, dependency changes, integration connection changes, and running state remain visible, and its Refresh action explicitly rebuilds the catalog projection. Only available entries are injected into ProductManager prompts.

## Schedules Page

The top-level Schedules page is the schedule management surface for generated and backend-owned services. The table uses fixed column allocations so status, timestamp, and action changes do not shift the column boundaries; narrower viewports scroll the table instead of redistributing it. The Service column shows the service name and links generated services to Skill Detail; those links use the same hidden blue reveal arrow as Skills rows, while non-navigable platform schedules show no arrow. The Status column shows `active` or `paused`, replacing that value with the `running` indicator while the generated or platform schedule is executing. The Schedule column contains only the human-readable recurrence. Recurrence and next/last timestamps omit timezone names and abbreviations from the table; the Edit modal retains the timezone field that controls execution. Every service row supports edit, enable/disable, and Run Now. Generated and ordinary platform Run Now actions are available only while enabled. Assistant assessment uses the same Enable/Disable and Run Now endpoints as Agents → Assistant, permits manual runs while disabled, and its Edit action opens the shared schedule form in a modal with an interval default of every 3 days. Actions are left-aligned, Enable and Disable share one stable width, and ordinary Edit actions open the existing schedule form in a modal. There are no create, delete, or schedule-approval controls because every service owns exactly one required schedule. Last Run places a status-colored dot before the timestamp and exposes the status text from the dot's hover tooltip instead of repeating it inline. Schedule rows keep one consistent desktop height, including rows with action buttons. New backend registry services inherit the action set automatically.
...
`/settings/models` labels the existing blueprint route **Project planning and clarification**. It loads the live App Server model catalog and lets the user choose model and reasoning effort independently under **Agent Routing** for Act, Observer, and Assistant assessment, followed by ProductManager actions, the single-Codex Builder, Builder difficulty tiers plus repair/update, and Tester task/final/update actions. Model controls have an adjacent **Save model routing** action and show whether changes are unsaved or when the persisted routes were last saved. Unsupported model/effort combinations are rejected by the backend.
...
The Integrations subpage includes a Quercus panel with write-only token management, validated account identity, accessible active/invited/completed course checkboxes, exact selection saving, per-course sync and independent processing status/counts, skipped-file counts, and a square trash action beside every course. A global file-processing selector offers disabled or Marker + Surya with local llama.cpp, accepts the absolute llama.cpp installation directory, shows installation readiness and whether the on-demand server is currently running, and reports aggregate conversion state. Failed files remain terminal across ordinary sync, backfill, and startup recovery; a **Reprocess failed files** button explicitly queues them again and is enabled only when Marker is active, failures exist, and no pass is pending or running. Interrupted pending work still resumes automatically. The UI initially uses processing and locally stored course state returned by the connection status; the page Refresh action is the only UI action that requests a refreshed remote course list for display. Processing status refreshes every three seconds only while aggregate processing is `pending` or `running`. Failed health requests display `Backend unavailable` while an active poll continues checking for recovery. The Courses section starts collapsed behind a native disclosure with the shared rotating chevron. Selection changes queue one coalesced background synchronization; enabling Marker queues a coalesced backfill. Deselection retains local knowledge. Every delete action opens an Eidolon-rendered confirmation modal, never a browser-native confirmation request. Confirmed deletion removes selected, retained, or untracked courses from the list and deletes the complete local directory plus all synchronization records when present. Only the course currently being deleted is disabled; active synchronization or processing returns a prompt conflict instead of leaving the delete request waiting on its lock.

The Memory page includes an **Open agent folder** action that asks the backend to open the fixed local Act root. It does not expose a selectable or caller-controlled path.

The Integrations subpage also includes one shared Notion connection panel. The credential subsection comes first and adds or replaces the write-only private-connection token. A separate data-source subsection follows with Todo and Reports ID fields plus one Save and one Delete action that apply to both IDs. A third subsection configures one standalone Daily Feed page ID with its own Save and Delete actions. Source Save validates both exact schemas with the stored token before replacing either ID; Daily Feed Save validates only that page; each Delete clears its own configuration but preserves the credential. The panel shows sanitized bot/workspace identity, all configured resources, status, and validation time. It explicitly states that Notion remains the Todo, Report, and Daily Feed UI and that Eidolon has no local copy, cache, or sync controls. The token is cleared after every submission and never redisplayed.
...
A separate Microsoft/Outlook panel owns a write-only Microsoft OAuth client id and secret, displays the fixed `common` authority and Outlook redirect URI, and keeps client fields disabled while Outlook is connected. It documents authorization-code PKCE, account selection, the `openid profile email offline_access User.Read Mail.ReadWrite Mail.Send` scopes, plain-text Graph reads, and possible school/work administrator consent. It shows sanitized default-account status and Connect, Reconnect, Disconnect, and client-removal actions. Disconnect and client-removal use Eidolon-rendered confirmation modals; no browser-native confirmation API or public account selector is used. OAuth callbacks return only a bounded `outlook=connected|denied|failed` marker.

One Telegram panel contains separate **Notification / Approval**, **Act Agent**, **Observer Agent**, and **Assistant Agent** bot subsections. Each has its own write-only token, sanitized bot/pairing state, one-time private-chat instructions, and replacement/disconnect controls. The notification bot subsection includes the complete-input cloud privacy disclosure and transports Assistant plan approvals. Each conversational bot uses one Eidolon session per Telegram private-chat topic; the panel instructs the user to enable Threaded mode in BotFather. `/help` explains topics and `/status` reports the current topic; the old session-navigation commands are not exposed. Topic-less General messages do not create a hidden session. Pairing distinguishes `Awaiting private chat` from `Pairing expired`, and only reports connected after both private chat and user IDs are bound.

The integrations page also exposes a separate **WeCom Observer** panel with write-only Bot ID/Secret inputs, sanitized WebSocket connection status, a paired-user list, and connect/disconnect controls. **Add user** opens an Eidolon modal containing the short-lived `/pair` code. Each user has one current Observer conversation; `clear conversation` in WeCom starts fresh while preserving backend history. The panel does not expose session selection or history because WeCom has no such UI, and it does not expose agent selection or callable integration authorization because WeCom has no registry operations.
...
Runtime approval uses the existing `PermissionRequestModal`. Its single review shows each provider, operation id, read/write behavior, normalized scope, current authorization state, and per-operation connection availability alongside the base manifest permissions. Notion has no caller-selected scope; Todo list and Report list/get are low-risk reads while Todo create/update/delete, Report create/delete, and Daily Feed write are medium-risk writes. Google Calendar likewise has no caller-selected scope: list/get are low-risk reads and create/update/delete are medium-risk writes, all fixed to `primary`. Connection state alone never marks a skill approved.
