...
One Telegram panel contains separate **Notification / Approval**, **Act Agent**, **Observer Agent**, and **Assistant Agent** bot subsections. Each has its own write-only token, sanitized bot/pairing state, one-time private-chat instructions, and replacement/disconnect controls. The notification bot subsection includes the complete-input cloud privacy disclosure and transports Assistant plan approvals. Each agent bot exposes `/new`, `/sessions`, and `/use <id>` for its own sessions; ordinary text repairs a stale selected-session pointer automatically. Pairing distinguishes `Awaiting private chat` from `Pairing expired`, and only reports connected after both private chat and user IDs are bound.

The integrations page also exposes a separate **WeCom Observer** panel with write-only Bot ID/Secret inputs, sanitized WebSocket connection status, a paired-user list, and connect/disconnect controls. **Add user** opens an Eidolon modal containing the short-lived `/pair` code. Each user row shows its independent selected Observer session and has a modal-confirmed remove action that preserves Observer history. The panel does not expose agent selection or callable integration authorization because WeCom has no registry operations.
