...
All three agents use `runtime/act/`. Common managed instructions explain `memory/`, `knowledge/`, and `workspace/`; role instructions are supplied per thread. Act can write workspace files and explicit, user-approved memory. Observer and Assistant have read-only filesystem access. The backend owns synchronized knowledge and `knowledge/assistant/plans/`.
