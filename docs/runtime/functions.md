...
Function skills and scheduled services call the selected `backend.codex.call` capability through `function_runtime_capabilities.call_codex`. The helper sends the ephemeral bounded-run bearer token through the private relay; the backend resolves the caller and active version from that token rather than trusting a submitted skill id. A service token is accepted only for a schedule-attributed service run. Callers may optionally provide a model, reasoning effort, and JSON `response_schema`; the backend validates the schema and passes the selected model and effort to Codex as the bounded request. A saved Skill Detail model-and-effort override applies to the current skill during the backend call; without a current skill context or override, the existing request/global/default model and effort behavior remains in force. Codex call/response and optional internet access remain governed by the active manifest and approved runtime permissions. The legacy `POST /skills/{skill_id}/codex` route remains available to trusted local callers for compatibility but is not the sandbox transport.
