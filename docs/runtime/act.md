...
Act has an independent dispatcher and managed App Server process, separate from Observer, Assistant, Product Manager, and account-usage reads. Managed processes refresh at turn boundaries and use authenticated session-specific MCP configuration. Act's named permission profile allows network access; Observer and Assistant remain network-disabled. Act also receives the explicitly allowlisted `playwright` host MCP server for browser automation; it is copied into every Act thread configuration and must report ready tools before the turn can start. Other inherited host MCP servers remain disabled. The separate Codex Desktop `@oai/sky` Computer Use package is not an Act capability and must not be probed from Act's shell. Live web search is enabled. Backend function availability, integration authorization, and per-call approval remain authoritative.
