...
- Limitations/Future implementations: Progress-aware idle timeout, whole-workflow execution budgets, streamed partial diagnostics, and safe partial recovery remain deferred together in TODO-012. Backend-owned dependency provisioning remains TODO-011.
...
- Limitations/Future implementations: A dedicated function registry, cross-skill invocation contract, and any function-facing interface remain deferred to TODO-009 and were not implemented.
...
- Limitations/Future implementations: Domain-level Docker egress enforcement remains deferred in TODO-010; dynamic Function registration and cross-skill invocation remain deferred in TODO-009. WebSockets are intentionally unsupported. The explicit `local/dev` fallback is less isolated than Docker and is labeled accordingly.
