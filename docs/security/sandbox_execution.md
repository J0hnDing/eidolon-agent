...
Backend-mediated Function, Codex, and GitHub integration calls use ephemeral capabilities derived from the currently running function or current web-application instance. No-internet Docker functions use a transient internal network and an allowlisted relay that forwards only Function registry discovery/invocation, the exact function Codex request, and the exact integration invocation request; the relay does not grant general backend or internet access. Web-application relays expose the corresponding exact instance-capability paths. The entrypoint receives the capability token, but its pre-run tests do not. GitHub credentials and authorization headers exist only in the trusted backend provider adapter and are never placed in a container environment.
