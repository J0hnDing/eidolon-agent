...
- exposes `PERSONAL_AGENT_SKILL_ID` and points `PERSONAL_AGENT_BACKEND_URL` at a trusted, allowlisted capability relay for approved skill code; the main backend URL is not exposed to generated Docker skill containers.
...
Backend-mediated Function, Codex, and GitHub integration calls use ephemeral capabilities derived from the currently running function or current web-application instance. Docker functions use an allowlisted relay that forwards only Function registry discovery/invocation, the exact function Codex request, and the exact integration invocation request; the relay does not grant general backend access. No-internet functions use a transient internal network. Functions with approved network domains use a transient non-internal Docker network so the same relay remains available alongside Docker internet egress. The main backend URL and common Docker host alias are not exposed to generated function containers. Web-application relays expose the corresponding exact instance-capability paths. The entrypoint receives the capability token, but its pre-run tests do not. GitHub credentials and authorization headers exist only in the trusted backend provider adapter and are never placed in a container environment.

Docker function and web-application containers map direct `api.github.com`, `github.com`, Google provider hosts including `gmail.googleapis.com`, Microsoft mail hosts `graph.microsoft.com` and `login.microsoftonline.com`, and `api.telegram.org` resolution to loopback even when ordinary runtime network access is approved. Combined with deterministic source validation, this keeps provider traffic on the trusted integration path. Generated skills must use the trusted provider-neutral email operations rather than Graph or Gmail URLs. Explicit local/development runner mode remains less isolated and is not a production secret-backed runtime boundary.
