...
The single Observer bot supports multiple paired users. Settings lists them and **Add user** opens an Eidolon modal containing a ten-minute one-time code. Only the code hash and expiry are persisted. A user pairs by sending `/pair <code>` from a private WeCom chat; unpaired users are ignored and pairing from groups is rejected. Pairing immediately creates exactly one active canonical Observer session and stores its `current_session_id`; every ordinary message uses it. WeCom does not expose session listing, switching, or history selection. `clear conversation` (or `/clear`) creates a fresh session and atomically replaces the current pointer before attempting to archive the old session. An idle old session is archived immediately; a busy one becomes unroutable immediately and is archived after its final queued or running turn finishes. Archiving the current session through Chat atomically creates and points to a fresh WeCom session for the same user before retiring the old one. Removing a user deletes only that binding; disconnecting removes the credential, all bindings, and message-deduplication records. Observer sessions and history are preserved in both cases.

If a migrated or externally archived current session is no longer active, WeCom status reconciliation or the next ordinary message creates and persists one fresh current session for that user. This is a WeCom-only binding rule; unrelated agent sessions have no default-session repair or cross-channel fallback.
