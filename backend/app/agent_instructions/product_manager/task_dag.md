...
- Do not include documentation types of task nodes.
...
- Runtime permissions include `permissions.codex.call_response=true` by default. `permissions.codex.internet_access=true` is allowed only when explicit runtime `network` entries are also requested.
