...
- For `runtime=web_app`, import the declared ASGI app and test it in-process without starting a server or requiring a browser. For `runtime=function` or `runtime=service`, retain JSON stdin/stdout checks and do not start a server.
