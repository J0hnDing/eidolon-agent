...
Use only operations in Selected integration operation context. Function code calls `integration_runtime_capabilities.call` with a literal operation id; web-application server code calls `web_runtime_capabilities.call_integration`. Tests use a deterministic fake adapter and no credential or live GitHub request. Never call GitHub directly, construct authentication headers, use internal integration HTTP paths, or place integration capability code in browser assets.

Treat the backend-seeded manifest runtime as an execution protocol. A `function` skill uses bounded JSON stdin/stdout. It may call only manifest-declared functions through `function_runtime_capabilities.call_function`; registry discovery never authorizes arbitrary function selection. A `web_app` exposes its declared importable ASGI entrypoint, owns its HTML/CSS/JavaScript inside the skill package, uses module-relative paths for read-only assets, and resolves persistent state from `PERSONAL_AGENT_SKILL_CACHE_DIR` (with `./cache` only as a development fallback). Never place writable cache beneath `__file__` or another package-relative path because the package is read-only at runtime. Test a web app in-process with its real cache-path contract; do not start a persistent server. Browser-side external URLs, custom Dockerfiles or commands, and edits to the Eidolon frontend are forbidden. Scoped server-side Codex calls use `web_runtime_capabilities.call_codex`, and declared server-side function calls use `web_runtime_capabilities.call_function`; never expose the instance capability to browser code.
